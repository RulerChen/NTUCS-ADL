...
from utils import jsonl_to_data
...
def main():
    ...
    if args.validation_file is not None:
        ...
        data = jsonl_to_data(args.train_file)
        all_datasets = datasets.Dataset.from_list(data)

        spilt_dataset = all_datasets.train_test_split(train_size=0.9, seed=args.seed)
        raw_datasets = datasets.DatasetDict(
            {"train": spilt_dataset["train"], "validation": spilt_dataset["test"]}
        )
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(eval_dataloader):
            if step % 100 == 0:
                logger.info(f"Step {step}, total {len(eval_dataloader)} steps")
            with torch.no_grad():
    ...
    if args.output_dir is not None:
        ...
        if accelerator.is_main_process:
            ...
            with open(os.path.join(args.output_dir, "plot.json"), "w") as f:
