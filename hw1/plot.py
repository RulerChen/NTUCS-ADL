...
def main():
    ...
    if args.dataset_name is not None:
        ...
        if args.validation_file is not None:
            ...
            extension = args.validation_file.split(".")[-1]
        # if args.test_file is not None:
        #     data_files_test = args.test_file
        #     extension = args.test_file.split(".")[-1]
    ...
    train_datasets = [
        {
            "id": raw_datasets["train"][i]["id"],
            "question": raw_datasets["train"][i]["question"],
            "context": context[raw_datasets["train"][i]["relevant"]],
            "answers": {
                "text": [raw_datasets["train"][i]["answer"]["text"]],
                "answer_start": [raw_datasets["train"][i]["answer"]["start"]],
            },
        }
        for i in range(len(raw_datasets["train"]))
    ]
    raw_datasets["train"] = datasets.Dataset.from_list(train_datasets)
    validation_datasets = [
        {
            "id": raw_datasets["validation"][i]["id"],
            "question": raw_datasets["validation"][i]["question"],
            "context": context[raw_datasets["validation"][i]["relevant"]],
            "answers": {
                "text": [raw_datasets["validation"][i]["answer"]["text"]],
                "answer_start": [raw_datasets["validation"][i]["answer"]["start"]],
            },
        }
        for i in range(len(raw_datasets["validation"]))
    ]
    raw_datasets["validation"] = datasets.Dataset.from_list(validation_datasets)
    ...
    # collect the training loss and EM metric
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            with accelerator.accumulate(model):
                ...
                # every 100 steps, we will log the loss and EM metric
                total_loss += loss.detach().float()

                if step % 500 == 0 and step != 0:
                    print("step: ", step)
                    total_loss = total_loss / len(train_dataloader)
                    train_loss_list.append(total_loss.cpu().numpy())
                    total_loss = 0
                    ...
                    for step, batch in enumerate(eval_dataloader):
                        with torch.no_grad():
                            ...
                            all_start_logits.append(
                                accelerator.gather_for_metrics(start_logits)
                                .cpu()
                                .numpy()
                            )
    ...
    plt.title("Exact Match metric")
    plt.xlabel("Steps")
    plt.ylabel("EM")
    plt.legend()

    plt.savefig(f"{args.output_dir}/em_metric.png")
    plt.close()
