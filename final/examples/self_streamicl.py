...
class LocalModelAgent(Agent):
    ...
    def generate_response(self, messages: list) -> str:
        ...
        model_inputs = self.tokenizer([text_chat], return_tensors="pt").to(
            self.model.device
        )
...
class ClassificationAgent(LocalModelAgent):
    ...
    def __call__(self, label2desc: dict[str, str], text: str) -> str:
        ...
        shots = (
            self.rag.retrieve(query=text, top_k=self.rag.top_k)
        ...
        if len(shots):
            ...
            try:
                prompt = re.sub(
                    pattern=r"\{fewshot_text\}",
                    repl=fewshot_text,
                    string=prompt_fewshot,
                )
            ...
            print(
                Fore.YELLOW + "No RAG shots found. Using zeroshot prompt." + Fore.RESET
            )
            prompt = prompt_zeroshot
        ...
        self.update_log_info(
            log_data={
                "num_input_tokens": len(
                    self.tokenizer.encode(self.get_system_prompt() + prompt)
                ),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="path to save csv file for kaggle submission",
    )
